...
export function transportReceiver() {
    amqp.connect(MESSAGE_QUEUE, function (err0, connection) {
        ...
        connection.createChannel(function (err1, channel) {
            ...
            const subscribed = [FACTORY_EXCHANGE_NAME];
            ...
            for (let i = 0; i < subscribed.length; i++) {
                ...
                channel.assertQueue('', {
                    ...
                    console.log(` [*] Transport MQ waiting for messages in queue: ${q.queue}`);
                    ...
                    channel.consume(q.queue, async function (message) {
                        if (message.content) {
                            ...
                            console.log(` [x] Transport MQ received: ${JSON.stringify(order)}`);
                            await worker.transportOrderWorker(order.order_id);
                        }
                    }, {
                        noAck: true
                    });
