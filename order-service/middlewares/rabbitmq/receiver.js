...
export function orderReceiver() {
    amqp.connect(MESSAGE_QUEUE, function (err0, connection) {
        ...
        connection.createChannel(function (err1, channel) {
            ...
            const subscribed = [CLIENT_EXCHANGE_NAME]
            ...
            for (let i = 0; i < subscribed.length; i++) {
                ...
                channel.assertQueue('', {
                    ...
                    console.log(` [*] Order MQ waiting for messages in queue: ${q.queue}`);
                    ...
                    channel.consume(q.queue, async function (message) {
                        if (message.content) {
                            ...
                            console.log(` [x] Order MQ received: ${JSON.stringify(order)}`);
                            await worker.confirmOrderWorker(order.order_id);
                        }
                    }, {
                        noAck: true
                    });
